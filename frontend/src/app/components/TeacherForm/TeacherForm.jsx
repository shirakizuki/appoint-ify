...
const TeacherForm = ({closeModal, departmentID, onClose}) => {
    ...
    const [error, setErrors] = useState([]) // State to store error message.
    const [success, setMessage] = useState('') // State to store the success message
    // CONTEXT OBJECT AND DESTRUCTUING
    const { url } = useContext(ServerContext) // Object context using useContext.
     ...
     */
    const resetForm = () => {
        setData(prevData => ({ // Reset the data state with initial values
            ...prevData,
            firstName: '',
            lastName: '',
            teacherEmail: '',
        ...
        setErrors([]);
        document.forms['teacher-form'].reset(); // Reset the form with a more efficient method
    }
    ...
    const clearFormOnClose = () => {
        ...
        onClose() // Call onClose function passes as a prop
    }
