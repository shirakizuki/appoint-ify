...
const defaultEmail = process.env.MAILERSEND_EMAIL;
...
export function createDeclineMessage(email, cancelReason) {
    const message = {
        ...
        html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e0e0e0; border-radius: 10px; background-color: #f9f9f9;">
            <h2 style="text-align: center; color: #333;">Your Appointment Has Been Declined</h2>
            ...
            <p style="color: #555;">Please coordinate with the department regarding your appointment. Or book another appointment.</p>
            <p style="color: #555;">Thank you for using Appointify! We look forward to serving you.</p>
...
// ACTIVE -> CANCELLED
export function createCancelledMessage(email, cancelReason) {
    const message = {
        from: defaultEmail,
        to: email,
        subject: 'Appointment Approved',
